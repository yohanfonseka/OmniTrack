    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="space-y-4">
          {filteredCampaigns.map(camp => {
            ...
            return (
              <div
                key={camp.id}
                className="bg-white rounded-2xl border border-slate-200 p-5 shadow-2xs hover:border-slate-300 transition-all flex flex-col lg:flex-row lg:items-center justify-between gap-6"
              >
                {/* Left: Platform & Campaign Identity */}
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] font-mono text-slate-500 bg-slate-50 px-2 py-0.5 rounded border border-slate-200 flex items-center gap-1">
                    ...
                    {camp.platform_account_name && (
                      <span className="text-[10px] text-slate-500">
                        Account: <strong className="text-slate-700">{camp.platform_account_name}</strong>
                      </span>
                    )}
                  ...
                  <h3 className="text-base font-bold text-slate-900 tracking-tight leading-snug">
                    {camp.platform_campaign_name}
                  </h3>

                  {/* Date Range & Ingestion metadata */}
                  <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 mt-2">
                    ...
                    <span>•</span>
                    <span>{camp.row_count || 1} daily report records</span>
                    <span>•</span>
                    <span>Pulled: {new Date(camp.pulled_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>

                  {/* If already mapped, show target info */}
                  {isMapped && camp.mapped_campaign_id && (
                    <div className="mt-3 p-2.5 bg-emerald-50/70 border border-emerald-200/80 rounded-xl text-xs text-emerald-900 flex items-center gap-2">
                ...
                {/* Middle: Performance Metrics Block */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 py-3 px-4 bg-slate-50/80 rounded-xl border border-slate-200/80 shrink-0 lg:w-[420px]">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">
                      Spend
                    </span>
                    <span className="text-sm font-extrabold text-slate-900">
                      {camp.currency} {camp.total_spend.toLocaleString()}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-slate-700">
                      {camp.total_clicks.toLocaleString()} <span className="text-[10px] text-slate-400">({ctr}%)</span>
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-emerald-600">
                      ...
                      {cpa > 0 && <span className="text-[10px] text-slate-500 block font-normal">CPA: {camp.currency} {cpa.toLocaleString()}</span>}
                    </span>
                  </div>
                </div>
                  ...
                  {camp.status === 'unmapped' ? (
                    <>
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-xs hover:shadow transition-all cursor-pointer"
                      >
      ...
      {selectedForMapping && (
        <MapCampaignModal
          ...
          onSuccess={() => {
            showToast(`Campaign successfully mapped to system!`);
            fetchCampaigns();
          }}
        />
      )}
